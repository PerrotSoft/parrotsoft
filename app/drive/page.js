...
export default function ParrotDrive() {
    ...
    const save = async (newData) => {
        const prevData = drive; // на случай отката при превышении лимита
        setDrive(newData);
        if (window.syncDrive) {
            const res = await window.syncDrive(user, newData);
            if (res && res.success === false && res.error === 'quota_exceeded') {
                setDrive(prevData); // откатываем оптимистичное обновление — на сервере ничего не сохранилось
                const usedMB = (res.usedBytes / (1024 * 1024)).toFixed(1);
                const quotaMB = (res.quotaBytes / (1024 * 1024)).toFixed(0);
                alert(`Превышен лимит места на диске: ${usedMB} МБ из ${quotaMB} МБ. Удали что-нибудь, чтобы освободить место.`);
            }
        }
    };
    ...
    // Скачать файл — та же расшифровка/распаковка, что и в handleView,
    // но вместо предпросмотра сразу отдаём файл браузеру на сохранение.
            ...
            <header className="p-header">
                ...
                {selected.size === 1 && selectedItem && (
                    <div className="p-selection-tools">
                        ...
                        {selectedItem.data && (
                            <button onClick={() => handleDownload(selectedItem)}>⬇️ Download</button>
                        )}
                        <button onClick={() => {
            ...
            {contextMenu && (
                <div className="p-ctx" style={{ top: contextMenu.y, left: contextMenu.x }}>
                    ...
                    {!contextMenu.isFolder && (
                        <button onClick={() => {
                            const f = (drive?.files || []).find(x => x.id === contextMenu.id);
