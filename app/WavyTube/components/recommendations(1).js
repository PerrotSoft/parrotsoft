...
export function getGlobalRecommendations(videos) {
  if (!videos) return [];
  return [...videos].map(v => {
    let score = 0;
    score += (v.views || 0) * 0.2;
...
export function RecommendationSystem({ videos, currentVideo, currentUsername, onPlay, onSelectChannel }) {
  const playlistName = currentVideo?.playlist;
  const [viewMode, setViewMode] = useState('recommendations');
  const [playlistSort, setPlaylistSort] = useState('recommended');
  const [personalizedFeed, setPersonalizedFeed] = useState(null); // null = ещё не загружено/недоступно → используем клиентский расчёт

  // Персональная лента (косинусное сходство векторов интересов, см.
  // actions.getPersonalizedFeed) — только когда реально смотрим рекомендации
  // (не плейлист) и известен пользователь. Для гостя remains null — тогда
  // ниже используется прежний клиентский подсчёт (getRecommendedVideos).
  useEffect(() => {
    let cancelled = false;
    if (viewMode === 'recommendations' && currentUsername) {
      actions.getPersonalizedFeed(currentUsername).then(feed => {
        if (!cancelled && feed && feed.length) {
          const excludeId = currentVideo?.id;
          setPersonalizedFeed(feed.filter(v => v.id !== excludeId));
        }
      }).catch(() => { if (!cancelled) setPersonalizedFeed(null); });
    }
    return () => { cancelled = true; };
  }, [viewMode, currentUsername, currentVideo?.id]);

  const playlistVideos = playlistName && playlistName !== 'Общее' ? videos.filter(v => v.playlist === playlistName) : [];
  const clientRecs = getRecommendedVideos(videos, currentVideo, viewMode === 'playlist' ? playlistSort : 'recommended_all');
  const recs = (viewMode === 'recommendations' && personalizedFeed) ? personalizedFeed : clientRecs;
