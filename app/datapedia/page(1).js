    ...
    return (
        <div className="dp-root">
            ...
            <main className="dp-main">
                <div className="dp-content-wrap">
                    ...
                    <div style={{ margin: '32px 0', display: 'flex', justifyContent: 'center' }}>
                        <AdSlot weight="light" type="468x60" />
                    </div>
            ...
            <style jsx global>{`
                ...
                /* Локальные токены темы — светлые по умолчанию, тёмные
                   переопределяются через уже существующий на <html>
                   атрибут data-theme="dark" (см. layout.js). Раньше вся эта
                   страница была зашита хардкодом на чёрную тему и не
                   реагировала на переключатель темы вообще. */
                .dp-root {
                    --dp-bg: #f7f7f9;
                    --dp-panel: #ffffff;
                    --dp-panel-2: #f0f1f4;
                    ...
                    --dp-text: #17181c;
                    --dp-text-dim: #6b6d78;
                    --dp-text-dim-2: #9698a3;
                    --dp-input-bg: #ffffff;
                    --dp-invert-bg: #17181c;
                    --dp-invert-text: #ffffff;
                }
                [data-theme="dark"] .dp-root {
                    --dp-bg: #080808;
                    --dp-panel: #111111;
                    --dp-panel-2: #1a1a1a;
                    --dp-border: #222222;
                    ...
                    --dp-text: #eeeeee;
                    --dp-text-dim: #888888;
                    --dp-text-dim-2: #555555;
                    --dp-input-bg: #0a0a0a;
                    --dp-invert-bg: #ffffff;
                    --dp-invert-text: #000000;
                }

                .dp-root { display: flex; height: 100vh; background: var(--dp-bg); color: var(--dp-text); font-family: 'Inter', system-ui, sans-serif; }
                .dp-sidebar { width: 280px; background: var(--dp-panel); border-right: 1px solid var(--dp-border); display: flex; flex-direction: column; }
                ...
                .dp-logo-cube { width: 24px; height: 24px; background: var(--dp-invert-bg); color: var(--dp-invert-text); display: flex; align-items: center; justify-content: center; border-radius: 4px; font-size: 0.8rem; }

                .dp-search-bar { padding: 0 10px 20px; }
                .dp-search-bar input { width: 100%; background: var(--dp-panel-2); border: 1px solid var(--dp-border-2); padding: 10px; border-radius: 8px; color: var(--dp-text); outline: none; font-size: 0.85rem; }
                ...
                .dp-nav-item:hover { background: var(--dp-panel-2); }
                .dp-nav-item.active { background: var(--dp-invert-bg); color: var(--dp-invert-text); }
                .dp-nav-name { font-size: 0.9rem; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
                .dp-nav-owner { font-size: 0.7rem; color: var(--dp-text-dim-2); }
                .dp-nav-item.active .dp-nav-owner { color: var(--dp-text-dim); }

                .dp-nav-level { border-left: 1px solid var(--dp-border); margin-top: 5px; margin-bottom: 5px; }
                .dp-nav-chevron { margin-left: auto; font-size: 0.7rem; color: var(--dp-text-dim-2); }
                .dp-back-btn { background: var(--dp-panel-2); border: 1px solid var(--dp-border-2); color: var(--dp-text-dim); cursor: pointer; padding: 8px 12px; border-radius: 6px; font-size: 0.8rem; margin-bottom: 10px; display: inline-flex; transition: 0.2s; margin-left: 10px; }
                .dp-back-btn:hover { color: var(--dp-text); background: var(--dp-border); }
                .dp-stack-title { font-size: 0.7rem; color: var(--dp-text-dim); text-transform: uppercase; padding: 0 10px 10px; border-bottom: 1px solid var(--dp-border); margin-bottom: 10px; margin-left: 10px; font-weight: 700; letter-spacing: 1px; }

                .dp-side-bottom { padding: 20px; border-top: 1px solid var(--dp-border); }
                .dp-btn-new { width: 100%; background: var(--dp-invert-bg); color: var(--dp-invert-text); border: none; padding: 12px; border-radius: 8px; font-weight: 700; cursor: pointer; font-size: 0.9rem; transition: 0.2s; }
                .dp-btn-new:hover { opacity: 0.8; }

                .dp-main { flex: 1; overflow-y: auto; padding: 60px 40px; background: radial-gradient(circle at 0% 0%, var(--dp-panel), var(--dp-bg)); }
                .dp-content-wrap { max-width: 800px; margin: 0 auto; }
                .dp-doc-header { margin-bottom: 40px; border-bottom: 1px solid var(--dp-border); padding-bottom: 30px; }

                /* НОВОЕ РАСПОЛОЖЕНИЕ КНОПОК */
                .dp-header-meta { display: flex; flex-direction: column; gap: 12px; margin-bottom: 15px; }
                .dp-doc-path { font-size: 0.75rem; color: var(--dp-text-dim-2); text-transform: uppercase; letter-spacing: 1px; }
                .dp-doc-actions { display: flex; gap: 10px; align-items: center; justify-content: flex-start; }

                .dp-edit-btn, .dp-delete-btn { background: var(--dp-panel-2); border: 1px solid var(--dp-border-2); color: var(--dp-text-dim); padding: 6px 14px; border-radius: 6px; cursor: pointer; font-size: 0.8rem; transition: 0.2s; display: flex; align-items: center; gap: 5px;}
                .dp-edit-btn:hover { color: var(--dp-text); border-color: var(--dp-text-dim-2); background: var(--dp-border); }
                ...
                .dp-root h1 { font-size: 3.5rem; font-weight: 900; margin: 0; letter-spacing: -2px; line-height: 1; color: var(--dp-text); }

                .dp-root .markdown-body { color: var(--dp-text-dim); line-height: 1.8; font-size: 1.1rem; }
                .dp-root .markdown-body h2 { color: var(--dp-text); border-bottom: 1px solid var(--dp-border); padding-bottom: 8px; margin-top: 2em; }

                /* СТИЛИ КОММЕНТАРИЕВ */
                .dp-comments-section { margin-top: 60px; padding-top: 30px; border-top: 1px solid var(--dp-border); }
                .dp-comments-section h3 { font-size: 1.2rem; margin-bottom: 20px; color: var(--dp-text); }
                .dp-comments-list { display: flex; flex-direction: column; gap: 15px; margin-bottom: 25px; }
                .dp-comment { background: var(--dp-panel); border: 1px solid var(--dp-border); padding: 15px; border-radius: 10px; }
                .dp-comment-head { display: flex; justify-content: space-between; margin-bottom: 10px; font-size: 0.85rem; color: var(--dp-text-dim); }
                .dp-comment-head strong { color: var(--accent); }
                .dp-comment-text { color: var(--dp-text); font-size: 0.95rem; line-height: 1.5; white-space: pre-wrap; }

                .dp-comment-input { display: flex; flex-direction: column; gap: 10px; }
                .dp-comment-input textarea { width: 100%; height: 80px; background: var(--dp-panel-2); border: 1px solid var(--dp-border-2); color: var(--dp-text); padding: 12px; border-radius: 8px; resize: vertical; outline: none; font-family: inherit; }
                .dp-comment-input textarea:focus { border-color: var(--dp-text-dim-2); }
                .dp-comment-input button { align-self: flex-end; background: var(--dp-invert-bg); color: var(--dp-invert-text); border: none; padding: 8px 20px; border-radius: 6px; font-weight: 600; cursor: pointer; transition: 0.2s; }
                .dp-comment-input button:hover { opacity: 0.8; }
                .dp-login-prompt { background: var(--dp-panel-2); padding: 15px; text-align: center; border-radius: 8px; color: var(--dp-text-dim); font-size: 0.9rem; }

                .dp-modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.75); backdrop-filter: blur(10px); z-index: 1000; display: flex; align-items: center; justify-content: center; }
                .dp-modal { background: var(--dp-panel); width: 90vw; height: 85vh; border: 1px solid var(--dp-border-2); border-radius: 16px; display: flex; flex-direction: column; overflow: hidden; color: var(--dp-text); }
                .dp-modal-head { padding: 20px; border-bottom: 1px solid var(--dp-border); display: flex; justify-content: space-between; align-items: center; }
                .dp-close { background: none; border: none; color: var(--dp-text-dim-2); font-size: 2rem; cursor: pointer; transition: 0.2s; }
                .dp-close:hover { color: var(--dp-text); }
                ...
                .dp-input-row input { background: var(--dp-panel-2); border: 1px solid var(--dp-border-2); color: var(--dp-text); padding: 12px; border-radius: 8px; outline: none; flex: 1; }
                .dp-select-parent { background: var(--dp-panel-2); border: 1px solid var(--dp-border-2); color: var(--dp-text); border-radius: 8px; padding: 0 10px; outline: none; }
                .dp-editor-split { display: flex; flex: 1; gap: 20px; min-height: 0; }
                .dp-editor-split textarea { flex: 1; background: var(--dp-input-bg); border: 1px solid var(--dp-border-2); color: var(--dp-text); padding: 15px; border-radius: 8px; resize: none; font-family: monospace; outline: none; }
                .dp-preview { flex: 1; border: 1px solid var(--dp-border); border-radius: 8px; padding: 15px; overflow-y: auto; background: var(--dp-bg); }

                .dp-modal-foot { padding: 20px; border-top: 1px solid var(--dp-border); display: flex; justify-content: flex-end; }
                .dp-btn-pri { background: var(--dp-invert-bg); color: var(--dp-invert-text); border: none; padding: 10px 25px; border-radius: 6px; font-weight: 700; cursor: pointer; transition: 0.2s; }
                .dp-btn-pri:hover { opacity: 0.85; }

                .dp-mobile-menu-btn { display: none; position: fixed; top: 15px; left: 15px; z-index: 90; background: var(--dp-invert-bg); color: var(--dp-invert-text); border: none; border-radius: 8px; width: 40px; height: 40px; font-size: 20px; cursor: pointer; }
