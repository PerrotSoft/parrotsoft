...
export async function GET(request) {
    ...
    try {
        switch (cmd) {
            // Реальная проверка пароля (scrypt-хэш). Если у аккаунта пароль ещё
            // не задан — вернёт access:false (см. account_set_initial_password).
            case 'auth': return NextResponse.json({ access: await actions.verifyPassword(u, p) });
            case 'account_has_password': return NextResponse.json({ hasPassword: await actions.hasPasswordSet(u) });
            case 'account_set_initial_password': return NextResponse.json(await actions.setInitialPassword(u, args));
            case 'account_change_password': {
                // args в формате "старыйПароль:новыйПароль"
                const [oldPass, newPass] = args.split(':');
                return NextResponse.json(await actions.changePassword(u, oldPass, newPass));
            }
            ...
            case 'db_expand': {
                // args в формате "dbId:извМБ", например "pdb_abc123:5"
                const [dbId, extraMB] = args.split(':');
                return NextResponse.json(await actions.pdb_expandStorage(u, dbId, extraMB));
            }
            ...
            // OAuth: управление своими приложениями (client_id/secret)
            case 'oauth_register_app': {
                // args в формате "название:redirect_uri1,redirect_uri2"
                const [name, urisRaw] = args.split(':::');
                const uris = (urisRaw || '').split(',').map(s => s.trim()).filter(Boolean);
                return NextResponse.json(await actions.registerOAuthClient(u, name, uris));
            }
            case 'oauth_my_apps': return NextResponse.json(await actions.getMyOAuthClients(u));
            case 'oauth_delete_app': return NextResponse.json(await actions.deleteOAuthClient(args, u));
